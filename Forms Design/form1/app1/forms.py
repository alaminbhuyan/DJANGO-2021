...
# def checkOnlyDigits_ln2(value):
#     if value.isalnum():
#         raise forms.ValidationError(message="LastName can't contain any digits")

class UserForm(forms.Form):
    username = forms.CharField(widget=forms.TextInput(attrs={'class':'form-control', 'placeholder' : 'UserName'}), error_messages={'required':'Enter Your UserName'}, validators=[checkSpace, checkOnlyDigits])

    first_name = forms.CharField(widget=forms.TextInput(attrs={'class':'form-control', 'placeholder' : 'FirstName'}), error_messages={'required':'Enter Your First Name'}, validators=[checkOnlyDigits_fn])

    last_name = forms.CharField(widget=forms.TextInput(attrs={'class':'form-control', 'placeholder' : 'LastName'}), error_messages={'required':'Enter Your Last Name'}, validators=[checkOnlyDigits_ln])
    ...
    password = forms.CharField(widget=forms.PasswordInput(attrs={'class':'form-control', 'placeholder':'Password'}), error_messages={'required':'Enter Your Password'})

    password2 = forms.CharField(widget=forms.PasswordInput(attrs={'class':'form-control', 'placeholder':'Confirm Password'}), label="Confirm Password:", error_messages={'required':'Enter Your Confirm Password'})
